jQuery(document).ready(
    function ($) {
        ...
        $(document).on('click', "#gcmi-fb-addnew-filter",
            function () {
                $('#gcmi-fb-tabs').show();
                disableFilters();
                cleaningTabs();
                ...
                $("input[type='checkbox'][id='gcmi-fb-include-ceased']").removeAttr('checked');
                printTabsContent();
            }
        );
        // Click sul pulsante per modifica di un filtro esistente
        $(document).on('click', "button[id^='gcmi-fb-edit-filter-']",
            function () {
        ...
        $(document).on('click', "button[id^='gcmi-fb-delete-filter-']",
            function () {
                let delfiltername = $(this).attr('id').split("-").pop();
                let title = 'Conferma eliminazione filtro';
                let message = '<span class="ui-icon ui-icon-alert" style="float:left; margin:12px 12px 20px 0;"></span>' +
                    '<p>Vuoi davvero cancellare il filtro: <b>' + delfiltername + '</b>?</p>' +
                    'ATTENZIONE: Questa operazione non può controllare se il filtro è attualmente in uso nei tuoi moduli.';
        ...
        $(document).on('click', "#gcmi-fb-button-cancel",
            function () {
        ...
        $(document).on('click', "#gcmi-fb-button-save",
            function () {
                ...
                if(rawfiltername !== myfiltername) {
                    let title = "Errore nel salvataggio";
                    let message = '<span class="ui-icon ui-icon-notice" style="float:left; margin:12px 12px 20px 0;"></span>' +
                        'Il valore indicato per il nome del filtro:<b><i>' + rawfiltername + '</i></b> non è utilizzabile.<br>' +
                        'Vuoi utilizzare: <b>' + myfiltername + '</b> ?';
                    $.when(customConfirm(message, title)).then(
                        function () {
        ...
        $(document).on('change', "input[type='checkbox'][id^='fb-gcmi-reg-']",
            //$( "input[type='checkbox'][id^='fb-gcmi-reg-']" ).change(
            function () {
                var chk = $(this);
                var codreg = $(this).attr('id').split("-").pop();
                if(false === chk.prop('checked')) {
                    // disabilito le province della regione
                    $('#gcmi-fb-regione-blocco-' + codreg).find('input[type=checkbox][id^="fb-gcmi-prov-"]:checked').removeAttr('checked');
                    $('#gcmi-fb-regione-blocco-' + codreg).find('input[type=checkbox][id^="fb-gcmi-prov-"]').trigger('change');
                    ...
                    $('#gcmi-fb-regione-blocco-' + codreg).find('input[type=checkbox][id^="fb-gcmi-prov-"]:not(:checked)').prop('checked', true);
                    $('#gcmi-fb-regione-blocco-' + codreg).find('input[type=checkbox][id^="fb-gcmi-prov-"]').trigger('change');
        ...
        $(document).on('change', "input[type='checkbox'][id^='fb-gcmi-prov-']",
            //$( "input[type='checkbox'][id^='fb-gcmi-prov-']" ).change(
            function () {
                var chk = $(this);
                ...
                if(false === chk.prop('checked')) {
                    ...
                    $("[name^='gcmi-com-cod-pro-" + codprov).find('input[type=checkbox]:checked').removeAttr('checked');
                    // li nascondo
                    ...
                    // li abilito (difficile capire qui cosa gli utenti possono preferire)
                    $("[name^='gcmi-com-cod-pro-" + codprov).find('input[type=checkbox]:not(:checked)').prop('checked', true);
                    hideemptyletters();
                }
                // metto il check a checkall se sono tutte checked
                if($("[id='gcmi-fb-regione-blocco-" + codreg).find('input[type=checkbox][id^=fb-gcmi-prov-]:checked').length === $("[id='gcmi-fb-regione-blocco-" + codreg).find('input[type=checkbox][id^=fb-gcmi-prov-]').length) {
        ...
        $(document).on('change', "input[type='checkbox'][id^='fb-gcmi-com-']",
            function () {
                let letteraIniziale = Array.from( $("label[for='"+this.name+"']").text() )[0];
                // Rimuovo il check da checkall
                $("[id='fb-gcmi-chkallcom-" + letteraIniziale).removeAttr('checked');
                // metto il check a checkall se sono tutte checked
                if($("[id='gcmi-fb-lettera-blocco-" + letteraIniziale).find('input[type=checkbox][id^=fb-gcmi-com-]:visible:checked').length === $("[id='gcmi-fb-lettera-blocco-" + letteraIniziale).find('input[type=checkbox][id^=fb-gcmi-com-]:visible').length) {
        ...
        $(document).on('change', "input[type='checkbox'][id^='fb-gcmi-chkallpr-']",
            //$( "input[type='checkbox'][id^='fb-gcmi-chkallpr-']" ).change(
            function () {
                var chk = $(this);
                var codreg = $(this).attr('id').split("-").pop();
                if(false === chk.prop('checked')) {
                    // disabilito le province della regione
                    $('#gcmi-fb-regione-blocco-' + codreg).find('input[type=checkbox]:checked').not("[id^='fb-gcmi-chkallpr-']").removeAttr('checked');
                    $('#gcmi-fb-regione-blocco-' + codreg).find('input[type=checkbox]').not("[id^='fb-gcmi-chkallpr-']").trigger('change');
                } else {
                    $('#gcmi-fb-regione-blocco-' + codreg).find('input[type=checkbox]:not(:checked)').not("[id^='fb-gcmi-chkallpr-']").prop('checked', true);
                    $('#gcmi-fb-regione-blocco-' + codreg).find('input[type=checkbox]').not("[id^='fb-gcmi-chkallpr-']").trigger('change');
                }
            }
        );
        ...
        $(document).on('change', "input[type='checkbox'][id^='fb-gcmi-chkallcom-']",
            function () {
                ...
                if(false === chk.prop('checked')) {
                    ...
                    $('#gcmi-fb-lettera-blocco-' + lettera).find('input[type=checkbox]:checked:visible').not("[id^='fb-gcmi-chkallcom-']").removeAttr('checked');
                } else {
                    $('#gcmi-fb-lettera-blocco-' + lettera).find('input[type=checkbox]:not(:checked):visible').not("[id^='fb-gcmi-chkallcom-']").prop('checked', true);
                }
            }
        );
        
        // codice AJAX per il selettore cessati - trigger per la richiesta dati
        // qui devo gestire se sta modificando un filtro o meno
        $(document).on('change', "input[type='checkbox'][id='gcmi-fb-include-ceased']",
            function(){
                var realFilterName = $('#fb_gcmi_filter_name').val();
                var cdate = new Date();
                ...
                $.ajax({
                    ...
                    success: function (res) {
                        ...
                        printTabsEditFilter( tmpFilterName );
                        ...
                        // imposto al valore precedente il campo nome del filtro
                        $('#fb_gcmi_filter_name').val(realFilterName);
                    },
                    error: function (res) {
                        console.log( "Fallita creazione filtro temporaneo");
                        printTabsContent();
                    }
                });
            }
        ...
        function printTabsEditFilter( editfiltername ) {
            ...
            $.ajax({
                ...
                success: function (res) {
                    ...
                    $("#gcmi-fb-tabs-2 input[type=checkbox]:not(:checked)").not("[id^='fb-gcmi-chkall-']").each(function () {
                        $(this).change();
                    });
                    $("#gcmi-fb-tabs-3 input[type=checkbox]:not(:checked)").not("[id^='fb-gcmi-chkallpr-']").each(function () {
                        $(this).change();
                    });
...
//                    });
        ...
        function print_filters() {
            $.ajax({
                ...
                success: function (res) {
                    ...
                    $('#gcmi-fb-filters-container').append(res.filters_html);
                }
            });
        }
        ...
        function remove_accents(string) {
            ...
            if(locale_from_server.startsWith('de')) {
                ...
                chars['l·l'] = 'll';
            } else if(('sr_RS' === locale_from_server) || ('bs_BA' === locale_from_server)) {
                chars['Đ'] = 'DJ';
                chars['đ'] = 'dj';
            }
            let replaced_string = "";
        ...
        function hideemptyletters() {
            $("div[class^='gcmi-fb-lettera-blocco']").each(function () {
                var wrap = $(this);
                if(wrap.find("input[type=checkbox][id^='fb-gcmi-com-']:checked").length === 0) {
                    wrap.hide();
                } else {
                    wrap.show();
                }
